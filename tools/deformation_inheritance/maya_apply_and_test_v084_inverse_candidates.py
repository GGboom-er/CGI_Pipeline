...
REPORT_PATH = INFO_DIR / "v084_maya_inverse_candidate_test_report.json"
ERROR_ARRAYS_PATH = INFO_DIR / "v084_maya_inverse_error_arrays.npz"
...
CANDIDATES = [
    {
        ...
        "mesh": "A_V084TEST_000_baseHybrid",
        "group": "A_V084TEST_000_baseHybrid_GRP",
        "skin": "A_V084TEST_000_baseHybrid_skinCluster",
        "offset_x": -36.0,
        "material": "A_V084TEST_baseHybrid_MAT",
        "color": (0.05, 0.55, 0.2),
        "error_set": "CDFDIAG_V084_BASEHYBRID_JAW25_ERROR_TOP_SET",
    },
    {
        "key": "v084_inverse_strongBase_accepted",
        "weights_source": "v084",
        "mesh": "A_V084TEST_001_inverseStrongBase",
        "group": "A_V084TEST_001_inverseStrongBase_GRP",
        "skin": "A_V084TEST_001_inverseStrongBase_skinCluster",
        "offset_x": -12.0,
        "material": "A_V084TEST_inverseStrongBase_MAT",
        "color": (0.1, 0.35, 1.0),
        "error_set": "CDFDIAG_V084_STRONGBASE_JAW25_ERROR_TOP_SET",
    },
    {
        "key": "v084_inverse_balanced_accepted",
        "weights_source": "v084",
        "mesh": "A_V084TEST_002_inverseBalanced",
        "group": "A_V084TEST_002_inverseBalanced_GRP",
        "skin": "A_V084TEST_002_inverseBalanced_skinCluster",
        "offset_x": 12.0,
        "material": "A_V084TEST_inverseBalanced_MAT",
        "color": (0.85, 0.15, 0.85),
        "error_set": "CDFDIAG_V084_BALANCED_JAW25_ERROR_TOP_SET",
    },
    {
        "key": "v084_inverse_motion_accepted",
        "weights_source": "v084",
        "mesh": "A_V084TEST_003_inverseMotion",
        "group": "A_V084TEST_003_inverseMotion_GRP",
        "skin": "A_V084TEST_003_inverseMotion_skinCluster",
        "offset_x": 36.0,
        "material": "A_V084TEST_inverseMotion_MAT",
        "color": (1.0, 0.36, 0.04),
        "error_set": "CDFDIAG_V084_MOTION_JAW25_ERROR_TOP_SET",
    },
    {
        "key": "v085_inverse_motion_multiposeGated",
        "weights_source": "v085",
        "mesh": "A_V085TEST_001_motionMultiposeGated",
        "group": "A_V085TEST_001_motionMultiposeGated_GRP",
        "skin": "A_V085TEST_001_motionMultiposeGated_skinCluster",
        "offset_x": 60.0,
        "material": "A_V085TEST_motionMultiposeGated_MAT",
        "color": (0.15, 0.95, 0.9),
        "error_set": "CDFDIAG_V085_MOTION_MULTIPOSE_GATED_JAW25_ERROR_TOP_SET",
    },
]
...
def _assign_material(mesh, material_name, color):
    cmds.sets(mesh, edit=True, forceElement=_make_material(material_name, color))


def _delete_existing():
    for item in CANDIDATES:
        for name in (item["group"], item["mesh"], item["error_set"]):
            if cmds.objExists(name):
                cmds.delete(name)
...
def _metrics_for(mesh, val, source_data, source_neutral_points, source_jaw_points, target_neutral_points):
...
def main():
    ...
    hybrid_weights = np.load(str(HYBRID_WEIGHTS_PATH), allow_pickle=True)
    v084_weights = np.load(str(V084_WEIGHTS_PATH), allow_pickle=True)
    v085_weights = np.load(str(V085_WEIGHTS_PATH), allow_pickle=True)
    weight_sources = {"hybrid": hybrid_weights, "v084": v084_weights, "v085": v085_weights}
    ...
    _delete_existing()
    cmds.undoInfo(openChunk=True, chunkName="CDFDIAG_V084_inverse_candidate_test")
    try:
        for item in CANDIDATES:
            ...
            _apply_weights(
                ...
                item["skin"],
                influence_names,
                weight_sources[item["weights_source"]][item["key"]],
            )
        ...
        report = {
            ...
            "v084_weights_path": str(V084_WEIGHTS_PATH),
            "v085_weights_path": str(V085_WEIGHTS_PATH),
            "error_arrays_path": str(ERROR_ARRAYS_PATH),
            "jaw_pose": {"M_Jaw_A_ctrl.rotateX": 25.0},
            "candidates": {},
            "note": "Maya actual DG validation for v084 constrained inverse accepted weights. Metrics use distance_k64 canonical provenance.",
        }
        selected_sets = []
        error_arrays = {}
        ...
        for item in CANDIDATES:
            ...
            full_error = np.full((int(cmds.polyEvaluate(item["mesh"], vertex=True)),), np.nan, dtype=np.float64)
            full_error[ids] = errors
            error_arrays[item["key"] + "_supported_ids"] = ids.astype(np.int32)
            error_arrays[item["key"] + "_motion_error"] = full_error
