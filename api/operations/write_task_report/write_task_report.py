# api/operations/write_task_report/write_task_report.py
...
def _extract_receipt(entry: dict) -> dict:
    ...
    if isinstance(detail, dict):
        ...
        try:
            ...
            parsed = None
    if isinstance(parsed, dict) and ('api_id' in parsed or 'outputs' in parsed):
        # 标准 receipt
...
def _render_hold_banner(hold_api_id: str, hold_step_idx: int, hold_detail: str,
                        hold_remaining: str, dcc_type: str = 'maya') -> list:
    lines = [
        '> ⚠️ **旧版链式执行暂停记录**',
        '>',
        f'> 暂停于 Step {hold_step_idx}: `{hold_api_id}`',
    ...
    lines.append('### 可选操作')
    lines.append('')
    lines.append('1. **提交修复API** — 场景还在 DCC 内存中，可以直接操作')
