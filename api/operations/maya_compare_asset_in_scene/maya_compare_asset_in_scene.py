# api/operations/maya_compare_asset_in_scene/maya_compare_asset_in_scene.py
# ── Maya 场景内资产对比 ──
#
...
def _resolve_input_source(params):
    """节点化新名优先，兼容 sync 侧旧习惯。"""
    return (
        params.get('input_source')
        or params.get('source_abc')
        or params.get('source_info')
        or ''
    ).strip()
...
def execute(payload: dict) -> dict:
    ...
    if not input_source:
        return make_receipt(
            ...
            error='缺少必填参数 input_source/source_abc/source_info。'
        )
