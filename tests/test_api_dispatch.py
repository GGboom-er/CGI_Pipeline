...
from core.tasks import _step_dcc, _step_id
...
def test_api_background_reuses_worker_task_and_api_payload():
    ...
    with patch.object(internals, "_ensure_worker", return_value=(True, "ok")), \
         patch("core.service_manager.start_redis", return_value=True), \
    ...
    assert args[0] == "core.tasks.execute_api_operation"
...
def test_api_foreground_uses_existing_maya_foreground_client():
    with patch(
        "mcp_server.foreground_client.submit_foreground_task",
    ...
    assert payload["api_id"] == "maya.rig.reference.update"
    assert payload["parameters"]["foreground_port"] == 7009
