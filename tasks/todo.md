...
- [ ] 期4 · 收尾：整理层级 + 规范 shape/orig 名 + 删垃圾 orig。**post-compare 保留**（要前后 delta 确认拼装差别，不删不改）；可选增强——让 post-compare 顺带 diff UV。
- [ ] 期5 · 法线：先做"判翻转"手段（按 ABC 参考逐面比法线），确认翻转才反转+conform；仅静态/新建件需要，复用件免。
