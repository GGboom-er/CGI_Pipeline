...
#
# 哲学：chain/workflow 引擎只往 audit 写事实，这个 API 是"事实 → 人类报告"的单一呈现层。
# 改 md 格式只改这一个文件，不影响任何业务 API。
...
from cgi_pipeline.core.receipt import make_receipt, _status_icon, _format_elapsed, _now_str
...
def _extract_receipt(entry: dict) -> dict:
    ...
    api_id = entry.get('api_id', 'unknown')
    ...
    if isinstance(detail, dict):
        ...
        try:
            ...
            parsed = None
    if isinstance(parsed, dict) and ('api_id' in parsed or 'output' in parsed):
        # 标准 receipt
        if 'status' not in parsed and status:
            parsed['status'] = status.replace('STEP_', '')
        parsed.setdefault('api_id', api_id)
        return parsed
    ...
    # fallback 时把 api_id 带进 summary，保留排障线索
    if api_id and api_id != 'unknown':
        action_text = f'`{api_id}` — {action_text}'
    return {
        'api_id': api_id,
...
def _action_for(api_id: str) -> str:
    if api_id in _ACTION_BY_API:
        return _ACTION_BY_API[api_id]
    short_id = api_id.rsplit('.', 1)[-1]
    if short_id in _ACTION_BY_API:
        return _ACTION_BY_API[short_id]
    if api_id.endswith('_export_abc'):
        return 'convert'
    if api_id.endswith('_build_asset_info') or api_id.endswith('_get_scene_info'):
        return 'collect'
    if api_id.startswith('compare') or '_compare' in api_id:
        return 'compare'
    if api_id.startswith('save_') or api_id.startswith('rename_'):
        return 'save'
    if 'clean' in api_id or 'cleanup' in api_id or 'fix' in api_id:
        return 'edit'
    return 'default'
...
def _render_workflow_context(entries: list, workflow_task_id: str) -> list:
    ...
    """
    # 文件流转是调度事实，最终报告按 API step 顺序阅读，避免额外可见折叠块打断主线。
    return []


def _render_segment_context(seg_open_entry: dict) -> list:
    """段级上下文：只渲染"打开了哪个场景"。
...
def _render_step(receipt: dict, step_idx: int) -> list:
    ...
    """
    api_id = receipt.get('api_id', 'unknown')
...
def _resolve_run_dir(task_id: str, given: str) -> Path:
    ...
    if given:
        ...
        return d
    try:
        from cgi_pipeline.core.service_manager import is_redis_alive
        import redis
        if is_redis_alive():
...
def _render_segment(rc: dict, seg_idx: int, step_entries: list = None,
    ...
    # 段内上下文（参数沙盒化 + 开场景）
    lines.extend(_render_segment_context(seg_open_entry))
...
def execute(payload: dict) -> dict:
    ...
    if mode not in ('normal', 'workflow'):
        return make_receipt(
            'write_task_report', 'ERROR', t0,
            error=f'不支持的 mode: {mode}。只允许 normal 或 workflow。',
        )
    ...
    # mode='workflow' 走 SEGMENT_* 路径；normal 走 STEP_* 路径
    if mode == 'workflow':
        segment_entries = [
            e for e in entries
    ...
    if mode == 'workflow':
        for idx, e in enumerate(segment_entries):
            ...
            lines.extend(_render_segment(seg, idx, seg_steps, seg_open))
            units_rendered += 1
    else:
        for e in step_entries:
    ...
    # 写沙盒。运行时唯一报告固定为 REPORT.md；本 API 作为 audit 重建工具时也覆盖同一文件。
    out_path = run_dir / 'REPORT.md'
    out_path.write_text('\n'.join(lines), encoding='utf-8')

    return make_receipt(
        api_id='write_task_report',
        status='SUCCESS',
        start_time=t0,
