# cgi_pipeline/capabilities/maya/rig/maya_sync_rig_incremental/maya_sync_rig_incremental.py
# ── Rig-Asset 增量同步（纯几何）──
#
# 核心流程：
...
def _ensure_collectable_shape_orig(transform):
    ...
    from cgi_pipeline.hosts.maya.asset_info_collector import get_shape_orig
...
def _collect_rig_meshes(cache_group=None):
    ...
    from cgi_pipeline.hosts.maya.asset_info_collector import collect_scene_info
...
from cgi_pipeline.capabilities.maya.asset.maya_build_mesh_from_abc.maya_build_mesh_from_abc import (
...
def _relocate_rig_mesh(rig_dag, new_name, target_parent, rig_prefix):
    ...
    # 几何注入由 dispatch 调用统一 plug 写入路径完成。
...
def _derive_report_dir(rig_path: str, tex_src: str) -> str:
    ...
    return os.path.join(str(REPOSITORY_ROOT), "projects", project, f"{ts}_{safe_asset}_sync")
...
def execute(payload: dict) -> dict:
    ...
    try:
        from cgi_pipeline.core.config_loader import get_rig_sync_profile
        profile = get_rig_sync_profile(project)
    except Exception:
        from cgi_pipeline.core.config_loader import get_default_rig_sync_profile
    ...
    try:
        _, external_report = _load_compare_result(compare_result_value)
        if abc_path:
    ...
    try:
        ...
        try:
            from cgi_pipeline.core.compare_result_io import generate_report
        ...
        from cgi_pipeline.hosts.maya.asset_info_collector import get_deform_input
        ...
        try:
            with open(os.path.join(str(REPOSITORY_ROOT), "logs", "sync_crash.log"), "a", encoding="utf-8", errors="replace") as _f:
