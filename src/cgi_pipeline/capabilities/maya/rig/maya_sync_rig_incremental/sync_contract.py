...
API_ID = "maya_sync_rig_incremental"
...
def parse_sync_inputs(payload: dict) -> SyncInputs:
    ...
    return SyncInputs(
        ...
        source_abc=_text(params.get("source_abc")),
        source_info=_text(params.get("source_info")),
...
def validate_compare_result_payload(data: dict) -> dict:
...
def load_compare_result_file(compare_result_path: str) -> tuple[dict, dict]:
    with open(compare_result_path, "r", encoding="utf-8") as fp:
        data = json.load(fp)
    return data, validate_compare_result_payload(data)


def load_compare_result_value(compare_result: Any) -> tuple[dict, dict]:
    if isinstance(compare_result, dict):
        return compare_result, validate_compare_result_payload(compare_result)
