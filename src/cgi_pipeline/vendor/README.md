# cgi_pipeline/vendor — 第三方依赖打包目录

本目录包含 CGI Pipeline 运行所需的第三方 Python 库，随项目一起发布。
DCC（Maya/Blender）启动时由 `cgi_pipeline.core.bootstrap` 自动注入 `sys.path`，无需手动安装。
...
- 更新依赖：`mayapy -m pip install <pkg> --target src/cgi_pipeline/vendor --upgrade --no-user`，然后删除意外带入的 `numpy*`。
