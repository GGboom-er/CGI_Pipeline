...
from cgi_pipeline.core.receipt import _format_elapsed, _status_icon
...
def _strip_block_markers(report_path: str | Path) -> None:
    ...
    if not path.exists():
        return
    marker_re = re.compile(r"^\s*<!--\s*report:block:(?:start|end)\s+.*?-->\s*$")
...
def render_step_finished(step_context: Dict[str, Any], receipt: Dict[str, Any],
    ...
    outputs = receipt.get("output") if isinstance(receipt.get("output"), dict) else {}
...
def extract_receipt(detail: Any, api_id: str = "", status: str = "") -> Dict[str, Any]:
    ...
    def _normalize(rc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not rc.get("api_version"):
            try:
                ...
                rc["api_version"] = str(get_capability(rc["api_id"]).get("version", "1.0.0"))
            except Exception:
    ...
    if isinstance(detail, dict):
        if any(k in detail for k in ("summary", "output", "api_id")):
