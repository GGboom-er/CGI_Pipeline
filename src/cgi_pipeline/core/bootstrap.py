# cgi_pipeline/core/bootstrap.py
# ── CGI Pipeline 统一配置入口 ──
#
# 自动加载 .env，暴露所有 DCC 路径和项目配置。
# 所有模块只需 `from cgi_pipeline.core.bootstrap import cfg` 即可拿到一切。
#
# 用法:
...
from cgi_pipeline.paths import PACKAGE_ROOT, REPOSITORY_ROOT
...
class _Cfg:
    ...
    def __init__(self):
        ...
        # 注入 cgi_pipeline/vendor 第三方依赖（trimesh/rtree 等）。
        vendor_dir = str(PACKAGE_ROOT / 'vendor')
        import sys
    ...
    def server_root(self, project=''):
        """获取项目的服务器根路径"""
        from cgi_pipeline.core.config_loader import get_server_root
