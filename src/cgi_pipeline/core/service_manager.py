# cgi_pipeline/core/service_manager.py
# ── 统一服务生命周期管理器 ──
#
# 所有 Worker/Redis 的检测、启动、关闭逻辑在此集中。
# cgi_pipeline.dashboard.app 和 cgi_pipeline.server.internals 都调用这个模块，
# 消除重复代码。
...
def _worker_owner_path() -> Path:
    """Return the HTTP process record for the canonical Worker owner."""
    return RUNTIME_DIR / 'worker_cgi.owner.json'


def claim_worker_owner(owner_pid: int) -> None:
    """Record that one HTTP MCP process started the shared CGI Worker."""
    if owner_pid <= 0:
        raise ValueError('owner_pid must be positive')
    RUNTIME_DIR.mkdir(parents=True, exist_ok=True)
    path = _worker_owner_path()
    temporary = path.with_name(f'{path.name}.{os.getpid()}.tmp')
    temporary.write_text(json.dumps({'owner_pid': owner_pid}), encoding='utf-8')
    os.replace(temporary, path)


def get_worker_owner() -> int | None:
    """Return the current HTTP Worker owner PID when a valid record exists."""
    try:
        record = json.loads(_worker_owner_path().read_text(encoding='utf-8'))
...
def release_worker_owner(owner_pid: int) -> bool:
    """Remove an ownership record only when it still belongs to this process."""
    path = _worker_owner_path()
    if get_worker_owner() != owner_pid:
        return False
    path.unlink(missing_ok=True)
    return True


def shutdown_worker_owned_by(owner_pid: int) -> bool:
    """Stop the shared Worker only when the target HTTP process created it."""
    if get_worker_owner() != owner_pid:
        return False
    stopped = stop_worker(_CANONICAL_WORKER)
    if stopped:
        release_worker_owner(owner_pid)
    return stopped
...
def ensure_worker_available(dcc: str = 'maya') -> tuple[bool, str]:
    """Ensure the canonical Worker can accept a submission without a broadcast probe.

    HTTP startup and explicit service-status checks own the expensive Celery
    heartbeat validation. The API hot path only needs Redis plus a live managed
    Worker PID; a missing Worker still follows the normal verified start path.
    """
    dcc = _normalize_dcc(dcc)
    if dcc not in _WORKER_DCCS:
        return False, f'暂不支持的 worker 类型: {dcc}'
    ...
    pid_alive, pid = is_worker_alive(dcc)
    if pid_alive:
        return True, f'{dcc} Worker 可用 (PID={pid})'
...
def start_worker(dcc: str = 'maya', wait: bool = True, timeout_sec: float = 30.0) -> bool:
    ...
    cmd = [
        ...
        "-A", "cgi_pipeline.core.tasks", "worker",
...
def shutdown_all():
    ...
    try:
        from cgi_pipeline.core import dcc_factory
...
def get_celery_app():
    ...
    if _celery_instance is None:
        from cgi_pipeline.core.tasks import app as celery_app
