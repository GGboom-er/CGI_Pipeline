# cgi_pipeline/dashboard/app.py
# ── CGI Pipeline Web Dashboard ──
#
...
@app.get('/api/apis')
async def get_apis():
    from cgi_pipeline.core.capability_registry import get_all_capabilities
    apis = get_all_capabilities()
...
async def worker_status_api():
    ...
    from cgi_pipeline.core.service_manager import get_worker_health
    ...
    try:
        ...
        if api_id in ['maya_export_abc', 'blender_export_abc', 'export_abc']:
            ...
            if project and category and asset_name and stage:
                try:
                    from cgi_pipeline.core.config_loader import load_project_config as _lpc2
                    from cgi_pipeline.core.asset_resolver import AssetResolver as _AR2
                    _cfg2 = _lpc2(project)
                    _r2 = _AR2(_cfg2)
...
async def execute_graph(request: Request):
    ...
    try:
        _app = get_celery_app()
        _app.send_task(
            'cgi_pipeline.core.tasks.execute_api_chain',
            args=[payload],
            task_id=chain_task_id,
        )
        dispatched = True
        ...
        for cr in reversed(chain_results):
            ...
            try:
                ...
                if isinstance(receipt, dict):
                    report_path = receipt.get('report_path') or receipt.get('output', {}).get('report_path', '')
                    if report_path:
                        break
            except Exception:
                pass
